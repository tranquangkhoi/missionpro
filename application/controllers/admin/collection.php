...
class Collection extends CI_Controller {
	...
	public function index(){				
		...
		$html['title'] = 'Quản lý Danh mục bộ sưu tập';
		...
		$config["total_rows"] = $this->collection_model->record_count();				
		$config['per_page'] = 20;
		...
		$data['stt'] = $offset;		
		$data['collection_items'] = $this->collection_model->index($config["per_page"], $offset);			 
        $data['msg'] = $this->session->flashdata('flash_msg');
		$data['pagination'] = $this->pagination->create_links();		
		$this->load->view('admin/collection/'.$this->uri->segment(2).'_view',$data);
	...
	public function filter(){
		...
		$config["total_rows"] = $this->collection_model->record_count_filter($str_like);
		$config['per_page'] = 20;
		...
		$data['stt'] = $offset;		
		$data['collection_items'] = $this->collection_model->filter($config["per_page"], $offset, $str_like);			 
        $data['msg'] = $this->session->flashdata('flash_msg');
		$data['pagination'] = $this->pagination->create_links();
		$this->load->view('admin/collection/'.$this->uri->segment(2).'_view',$data);
	}
	...
	public function add(){		
		...
		$html['title'] = 'Thêm mới danh mục bộ sưu tập';
		$data['header'] = $this->load->view('admin/header_view',$html,true);
		$data['footer'] = $this->load->view('admin/footer_view','',true);		
		
		$data['f_collection'] = $this->collection_model->add();		
							
		//return alert
		$alert_arr = array(
			'0' => '-Hãy nhập tên của Danh mục<br>',
			'1' => '-Lỗi Upload ảnh lên máy chủ<br>',
			'2' => '-Thêm mới thành công.<br>'			
		);
		$code_msg = explode(',',$data['f_collection']['code_msg']);		
		$data['msg'] = '';
		foreach($code_msg as $key => $val){
			$data['msg'] .= $val<>'' ? $alert_arr[$val] : '';			
		}
		
		$this->load->view('admin/collection/'.$this->uri->segment(2).'add_view',$data);		
		
	}
	
	public function edit(){		
		$collection_id = is_numeric($this->uri->segment(4)) ? $this->uri->segment(4) : 0;			
		if(!$this->collection_model->check_id('collection','collection_id',$collection_id)){
		...
		$data['f_collection'] = $this->collection_model->edit();
		...
		$this->load->view('admin/collection/'.$this->uri->segment(2).'edit_view',$data);
	}
	...
	public function del(){				
		...
		//delete
		$alert_code = $this->collection_model->del();		
