...
class Autocode extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('admin/autocode_model');
	}
	...
	public function index(){		
		...
		$html['title'] = 'Danh sách các mã thay thế nội dung';
		...
		$config["total_rows"] = $this->autocode_model->record_count();				
		$config['per_page'] = 20;
		...
		$data['stt'] = $offset;		
		$data['autocode_items'] = $this->autocode_model->index($config["per_page"], $offset);			 
	...
	public function filter(){
		...
		$config["total_rows"] = $this->autocode_model->record_count_filter($str_like);
		$config['per_page'] = 20;
		...
		$data['stt'] = $offset;		
		$data['autocode_items'] = $this->autocode_model->filter($config["per_page"], $offset, $str_like);			 
        $data['msg'] = $this->session->flashdata('flash_msg');
		$data['pagination'] = $this->pagination->create_links();
		$this->load->view('admin/'.$this->uri->segment(2).'/'.$this->uri->segment(2).'_view',$data);
	}
	...
	public function add(){		
		...
		$data['f_autocode'] = $this->autocode_model->add();		
			
		//return alert
		$alert_arr = array(
			'0' => '- Hãy nhập mã thay thế<br>',
			'1' => '- Hãy nhập mô tả ngắn<br>',
			'2' => '- Hãy nhập nội dung thay thế<br>',			
			'3' => '- Thêm mới thành công.<br>'			
		);
		$code_msg = explode(',',$data['f_autocode']['code_msg']);		
		$data['msg'] = '';
		...
		//gen editor				
		$text_area_review = $this->editor->view_file(".editme","550","200","1106","200");
		$data['f_autocode']['text_area_review'] = $text_area_review;		
	...
	public function edit(){		
		...
		if(!$this->autocode_model->check_id('autocode','autocode_id',$autocode_id)){
		...
		$data['f_autocode'] = $this->autocode_model->edit();
	...
	public function delimg(){
		...
		if($this->autocode_model->delimg($autocode_id)){
			echo 'OK';
		}else{
			echo 'Error';
		}
	...
	public function del(){				
		...
		//delete
		$alert_code = $this->autocode_model->del();		
