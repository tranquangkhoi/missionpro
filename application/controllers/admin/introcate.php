...
class Introcate extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('admin/introcate_model');
	}
	...
	public function index(){		
		...
		$config["total_rows"] = $this->introcate_model->record_count();				
		$config['per_page'] = 20;
		...
		$data['stt'] = $offset;		
		$data['introcate_items'] = $this->introcate_model->index($config["per_page"], $offset);			 
	...
	public function filter(){
		...
		$config["total_rows"] = $this->introcate_model->record_count_filter($str_like);
		$config['per_page'] = 20;
		...
		$data['stt'] = $offset;		
		$data['introcate_items'] = $this->introcate_model->filter($config["per_page"], $offset, $str_like);			 
        $data['msg'] = $this->session->flashdata('flash_msg');
		$data['pagination'] = $this->pagination->create_links();
		$this->load->view('admin/'.$this->uri->segment(2).'/'.$this->uri->segment(2).'_view',$data);
	}
	...
	public function sortup(){
		...
		if(!$this->introcate_model->check_id('introcate','introcate_id',$introcate_id)){
		...
		$this->introcate_model->sortup($introcate_id);
	...
	public function sortdown(){
		...
		$this->introcate_model->sortdown($introcate_id);
		redirect('admin/'.$this->uri->segment(2).'/'.$this->uri->segment(5),'refresh');				
	}
	...
	public function add(){		
		...
		$data['f_introcate'] = $this->introcate_model->add();	
		...
		$code_msg = explode(',',$data['f_introcate']['code_msg']);		
		$data['msg'] = '';
	...
	public function edit(){		
		...
		$data['f_introcate'] = $this->introcate_model->edit();		
	...
	public function del(){				
		...
		//delete
		$alert_code = $this->introcate_model->del();		
		$msg = $alert_arr[$alert_code];			
		$this->session->set_flashdata('flash_msg', $msg);
        redirect('admin/introcate','refresh');
	}		
}