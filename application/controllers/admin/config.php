...
class Config extends CI_Controller {
	...
	public function index(){		
		...
		$config["total_rows"] = $this->config_model->record_count();				
		$config['per_page'] = 20;
		...
		$data['stt'] = $offset;		
		$data['config_items'] = $this->config_model->index($config["per_page"], $offset);			 
	...
	public function filter(){
		...
		$config["total_rows"] = $this->config_model->record_count_filter($str_like);
		$config['per_page'] = 20;
		...
		$data['stt'] = $offset;		
		$data['config_items'] = $this->config_model->filter($config["per_page"], $offset, $str_like);			 
        $data['msg'] = $this->session->flashdata('flash_msg');
		$data['pagination'] = $this->pagination->create_links();
		$this->load->view('admin/'.$this->uri->segment(2).'/'.$this->uri->segment(2).'_view',$data);
	}
	...
	public function sortup(){
		...
		if(!$this->config_model->check_id('config','config_id',$config_id)){
		...
		$this->config_model->sortup($config_id);
	...
	public function sortdown(){
		...
		$this->config_model->sortdown($config_id);
		redirect('admin/'.$this->uri->segment(2).'/'.$this->uri->segment(5),'refresh');				
	}
	...
	public function add(){		
		...
		$data['f_config'] = $this->config_model->add();	
		
		//return alert
		$alert_arr = array(
			'0' => '-Hãy nhập thông tin về công ty/cửa hàng<br>',			
			'1' => '-Thêm mới thành công.<br>'			
		);
		$code_msg = explode(',',$data['f_config']['code_msg']);		
		$data['msg'] = '';
	...
	public function edit(){		
		...
		$data['f_config'] = $this->config_model->edit();		
		
		//return alert
		$alert_arr = array(
			'0' => '-Hãy nhập Nick config<br>',						
	...
	public function del(){				
		...
		//delete
		$alert_code = $this->config_model->del();		
