...
class Category extends CI_Controller {
	...
	public function index(){				
		...
		$html['title'] = 'Quản lý Danh mục sản phẩm';
		...
		$data['stt'] = $offset;		
		$data['category_items'] = $this->category_model->index($config["per_page"], $offset);			 
        $data['msg'] = $this->session->flashdata('flash_msg');
		$data['pagination'] = $this->pagination->create_links();		
		$this->load->view('admin/category/'.$this->uri->segment(2).'_view',$data);
	...
	public function filter(){
		...
		$config["total_rows"] = $this->category_model->record_count_filter($str_like);
		$config['per_page'] = 20;
		...
		$data['stt'] = $offset;		
		$data['category_items'] = $this->category_model->filter($config["per_page"], $offset, $str_like);			 
        $data['msg'] = $this->session->flashdata('flash_msg');
		$data['pagination'] = $this->pagination->create_links();
		$this->load->view('admin/category/'.$this->uri->segment(2).'_view',$data);
	}
	...
	public function add(){		
		...
		$data['f_category'] = $this->category_model->add();		
							
		//return alert
		$alert_arr = array(
			'0' => '-Hãy nhập tên của Danh mục sản phẩm<br>',
			'1' => '-Lỗi Upload ảnh lên máy chủ<br>',
			'2' => '-Thêm mới thành công.<br>'			
		);
		$code_msg = explode(',',$data['f_category']['code_msg']);		
		$data['msg'] = '';
		...
		$this->load->view('admin/category/'.$this->uri->segment(2).'add_view',$data);		
	...
	public function edit(){		
		...
		$data['f_category'] = $this->category_model->edit();
	...
	public function del(){				
		...
		//delete
		$alert_code = $this->category_model->del();		
